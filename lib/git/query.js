...
function exGit(repoPath, args, cb, noEncoding, outputStream) {
...
GitQuery.prototype.getTree = function(treePath, cb) {
	exGit(this.repoPath, ['ls-tree', '-z', this.treeIsh, treePath], function(err, data) {
		...
		cb(err, data === null ? false : trees.concat(blobs));
	});
}
...
GitQuery.prototype.makeArchive = function(blobPath, outputStream) {
	exGit(this.repoPath, ['archive', '--format', 'zip', this.treeIsh, blobPath], function() {
		outputStream.end();
	}, true, outputStream);
}
