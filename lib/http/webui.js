...
exports.handle = function(req, res, repos, git, site) {
	...
	var startTime = process.hrtime(),
		...
		config = {
			...
			repos: repos,
			repo: params[0],
			operation: (params[0] ? (params[1] === undefined ? 'tree' : params[1]) : 'list').toLowerCase(),
	...
	res.render = function(view, locals) {
		...
		locals.branch = config.branch;
		locals.sshRemotePrefix = site.sshRemotePrefix || '';
		var remotePrefix = locals.remotePrefix = site.remotePrefix || '';
		...
		locals.commandUrl = function(command, commandPath, branch) {
			...
			if(command === 'tree' && branch === 'master' && (commandPath === '/' || commandPath === '')) {
				return remotePrefix + prefix;
			}

			return remotePrefix + path.join(prefix + '/' + command + '/' + branch, commandPath);
		}

		locals.resolveUrl = function(url) {
			return remotePrefix + url;
		}
		...
		if(config.repo) {
			config.gitQuery.getBranches((function(err, branches) {
